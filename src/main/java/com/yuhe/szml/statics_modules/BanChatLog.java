...
public class BanChatLog extends AbstractStaticsModule {
					...
					for (String col : LOG_COLS) {
						...
						try {
							if (col.equals("StartTime")) {
								String timeStr = timeFormat.format(timeFormat.parse(value + "000"));
								map.put("BanStartTime", timeStr);
							} else if (col.equals("BanTime")) {
								int endTime = Integer.parseInt(value) + Integer.parseInt(map.get("StartTime"));
								String timeStr = timeFormat.format(timeFormat.parse(Integer.toString(endTime * 1000)));
